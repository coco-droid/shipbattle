...
#include <string.h>
void BeforeCallbackD4r(SDL_Event *event) {
...
void AfterCallbackD4r(SDL_Event *event) {
    InitLogFile("logs.txt");
    if (event->type == SDL_MOUSEBUTTONDOWN) {
...
void CloseCallbackD4r(SDL_Event *event) {
    InitLogFile("logs.txt");
    if (event->type == SDL_MOUSEBUTTONDOWN) {
       *alive=0;
    }
}
void SkipCallbackD4r(SDL_Event *event){
    InitLogFile("logs.txt");
    if (event->type == SDL_MOUSEBUTTONDOWN) {
      ShowOption(first_window,first_renderer);
    }
}
void Dialog_4_russia(SDL_Window* window, SDL_Renderer* renderer) {
    // Initialize log
    InitLogFile("logs.txt");
    ...
    LogMessage("dialog 1 launch");
        ...
        RenderText(renderer,"Marine, project our vessel coordinates on the central display. Afterward, proceed to the missile control station and prepare for imminent combat operations. We're establishing a blockade in the Malacca Strait", image2Rect.x + 10, image2Rect.y + 10, textColor, 16, image2Rect.w - 20);
        ...
        int width3 = 30;
        int height3 = 30;
        CreateClickableElement(renderer, 558, 11, &width3, &height3, NULL, textColor, "medias/images/btn-close.png", CloseCallbackD4r, 12);
        CreateClickableElement(renderer, 12, 11, &width3, &height3, NULL, textColor, "medias/images/left-arrow.png", BeforeCallbackD4r, 12);
        CreateClickableElement(renderer, 558, 558, &width3, &height3, NULL, textColor, "medias/images/right-arrow.png", AfterCallbackD4r, 12);
        CreateClickableElement(renderer, 519, 558, &width3, &height3, NULL, textColor, "medias/images/left2-arrow.png", AfterCallbackD4r, 12);
        CreateClickableElement(renderer, 13, 562, &width3, &height3, NULL, textColor, "medias/images/skip.png",SkipCallbackD4r, 12);
