...
#include <string.h>
void BeforeCallbackD3(SDL_Event *event) {
...
void AfterCallbackD3(SDL_Event *event) {
    InitLogFile("logs.txt");
    if (event->type == SDL_MOUSEBUTTONDOWN) {
        LogMessage("Button 2 clicked!\n");
        ClearEvents();
        Dialog_4_usa(first_window,first_renderer);
    }
}

void CloseCallbackD3(SDL_Event *event) {
    InitLogFile("logs.txt");
    if (event->type == SDL_MOUSEBUTTONDOWN) {
       *alive=0;
    }
}
void SkipCallbackD3 (SDL_Event *event){
    InitLogFile("logs.txt");
    if (event->type == SDL_MOUSEBUTTONDOWN) {
      ShowOption(first_window,first_renderer);
    }
}
void Dialog_3_usa(SDL_Window* window, SDL_Renderer* renderer) {
    // Initialize log
    InitLogFile("logs.txt");
    ...
    LogMessage("dialog 1 launch");
        ...
        RenderText(renderer,"Commander, we've detected an inbound ballistic missile launch from the Russian fleet targeting our Aegis-equipped destroyers. Our Phalanx CIWS systems are online, but we need immediate authorization to engage and neutralize the threat", image2Rect.x + 10, image2Rect.y + 10, textColor, 16, image2Rect.w - 20);
        ...
        int width3 = 30;
        int height3 = 30;
        CreateClickableElement(renderer, 558, 11, &width3, &height3, NULL, textColor, "medias/images/btn-close.png", CloseCallbackD3, 12);
        CreateClickableElement(renderer, 12, 11, &width3, &height3, NULL, textColor, "medias/images/left-arrow.png", BeforeCallbackD3, 12);
        CreateClickableElement(renderer, 558, 558, &width3, &height3, NULL, textColor, "medias/images/right-arrow.png", AfterCallbackD3, 12);
        CreateClickableElement(renderer, 519, 558, &width3, &height3, NULL, textColor, "medias/images/left2-arrow.png", BeforeCallbackD3, 12);
        CreateClickableElement(renderer, 13, 562, &width3, &height3, NULL, textColor, "medias/images/skip.png",SkipCallbackD3, 12);
