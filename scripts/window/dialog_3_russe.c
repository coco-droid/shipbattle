...
#include "../../headers/window/dialog_4_russe.h"
#include "../../headers/window/dialog_3_russe.h"
#include <string.h>
void BeforeCallbackD3r(SDL_Event *event) {
...
void AfterCallbackD3r(SDL_Event *event) {
    InitLogFile("logs.txt");
    if (event->type == SDL_MOUSEBUTTONDOWN) {
        LogMessage("Button 2 clicked!\n");
        ClearEvents();
        Dialog_4_russia(first_window,first_renderer);
    }
}

void CloseCallbackD3r(SDL_Event *event) {
    InitLogFile("logs.txt");
    if (event->type == SDL_MOUSEBUTTONDOWN) {
       *alive=0;
    }
}
void SkipCallbackD3r(SDL_Event *event){
    InitLogFile("logs.txt");
    if (event->type == SDL_MOUSEBUTTONDOWN) {
      ShowOption(first_window,first_renderer);
    }
}
void Dialog_3_russia(SDL_Window* window, SDL_Renderer* renderer) {
    // Initialize log
    InitLogFile("logs.txt");
    ...
    LogMessage("dialog 1 launch");
        ...
        RenderText(renderer,"Commander, our surveillance systems have identified an incoming Tomahawk missile strike from the American fleet aimed at our Kirov-class battlecruisers. Our S-400 defense systems are primed, but we require immediate clearance to activate our electronic countermeasures and intercept the missiles.", image2Rect.x + 10, image2Rect.y + 10, textColor, 16, image2Rect.w - 20);
        ...
        int width3 = 30;
        int height3 = 30;
        CreateClickableElement(renderer, 558, 11, &width3, &height3, NULL, textColor, "medias/images/btn-close.png", CloseCallbackD3r, 12);
        CreateClickableElement(renderer, 12, 11, &width3, &height3, NULL, textColor, "medias/images/left-arrow.png", BeforeCallbackD3r, 12);
        CreateClickableElement(renderer, 558, 558, &width3, &height3, NULL, textColor, "medias/images/right-arrow.png", AfterCallbackD3r, 12);
        CreateClickableElement(renderer, 519, 558, &width3, &height3, NULL, textColor, "medias/images/left2-arrow.png", BeforeCallbackD3r, 12);
        CreateClickableElement(renderer, 13, 562, &width3, &height3, NULL, textColor, "medias/images/skip.png",SkipCallbackD3r, 12);
